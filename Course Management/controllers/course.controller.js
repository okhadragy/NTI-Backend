...
const canModifyCourse = (course, requesterId, requesterRole) => {
  ...
  if (requesterRole === 'instructor') {
    for (const instructor of course.instructors) {
      if (instructor.user.toString() == requesterId && instructor.role == 'owner') {
        console.log(`User ${requesterId} is the owner of course ${course._id}`);
        return true;
      }
    }
  }
...
const createCourse = async (req, res) => {
  try {
    ...
    const users = [];
    for (const inst of instructors) {
      const user = await User.findById(inst.user);
      if (!user) {
        removeUploadedFiles(req.files);
        return res.status(400).json({ status: "fail", message: `Instructor not found: ${inst.user}` });
      }
      users.push(user);
    }

    if (users.length !== instructors.length || users.some(u => u.role !== 'instructor')) {
      removeUploadedFiles(req.files);
...
const getAllCourses = async (req, res) => {
  try {
    ...
    const courses = await Course.find(filter)
      .skip(skip)
      .limit(limit)
      .populate('instructors.user', 'name email role')
      .populate('pageDiscounts')
      .select('-__v -discounts');

    const coursesWithAppliedDiscount = courses.map(course => {
    ...
    res.status(200).json({
      ...
      length: coursesWithAppliedDiscount.length,
      data: coursesWithAppliedDiscount,
    });
  } catch (error) {
...
const updateCourse = async (req, res) => {
  try {
    ...
    await updatedCourse.populate('instructors.user', 'name email role');
